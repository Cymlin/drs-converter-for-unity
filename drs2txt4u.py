        ...
        if thestep.find('long_point') is None: #Standard note
            ...
            if points[0].find('left_pos').text == thestep.find('left_pos').text: #Analyze the 0th point | Static long note
                ...
                if points[0].find('left_end_pos') is not None: #L/R slick after Static long note
                    if int(points[0].find('left_end_pos').text) > int(points[0].find('left_pos').text): #R slick
                        ...
                        result += [(f'LNote{note_color[note_type]}', points[0].find('tick').text[:-1], pos_track[points[0].find('right_pos').text], pos_track[points[0].find('right_end_pos').text])] #Type, time frame, start track, end track
            elif points[0].find('left_pos').text != thestep.find('left_pos').text: #Moving Long note
                result += [(f'PYLongNote{note_color[note_type]}', thestep.find('start_tick').text[:-1], str(int(points[0].find('tick').text[:-1]) - int(thestep.find('start_tick').text[:-1])), pos_track[thestep.find('right_pos').text], pos_track[points[0].find('right_pos').text], pos_track[thestep.find('left_pos').text])]#Type, time frame, duration, start trackR, end track, start trackL
            for x in range(1, len(points)): #Analyze the 1st~ point
                if points[x].find('left_pos').text == points[x-1].find('left_pos').text and points[x-1].find('left_end_pos') is None: #Mid static long note
                    result += [(f'ZLongNote{note_color[note_type]}', points[x-1].find('tick').text[:-1], str(int(points[x].find('tick').text[:-1]) - int(points[x-1].find('tick').text[:-1])), pos_track[points[x].find('right_pos').text], pos_track[points[x].find('left_pos').text])]#Type, time frame, duration, trackR, trackL
                    ...
                    result += [(f'ZPYLongNote{note_color[note_type]}', points[x-1].find('tick').text[:-1], str(int(points[x].find('tick').text[:-1]) - int(points[x-1].find('tick').text[:-1])), pos_track[points[x-1].find('right_pos').text], pos_track[points[x].find('right_pos').text], pos_track[points[x-1].find('left_pos').text])]#Type, time frame, duration, start trackR, end track, start trackL
                    ...
                    if points[x].find('left_pos').text == points[x-1].find('left_end_pos').text: #Exception for mid static long note
                        ...
                        result += [(f'ZPYLongNote{note_color[note_type]}', points[x-1].find('tick').text[:-1], str(int(points[x].find('tick').text[:-1]) - int(points[x-1].find('tick').text[:-1])), pos_track[points[x-1].find('right_end_pos').text], pos_track[points[x].find('right_pos').text], pos_track[points[x-1].find('left_end_pos').text])]
                if points[x].find('left_end_pos') is not None: #L/R slick after mid long note
                    if int(points[x].find('left_end_pos').text) > int(points[x].find('left_pos').text): #R slick
                        ...
                        result += [(f'LNote{note_color[note_type]}', points[x].find('tick').text[:-1], pos_track[points[x].find('right_pos').text], pos_track[points[x].find('right_end_pos').text])] #Type, time frame, start track, end track
            return result
